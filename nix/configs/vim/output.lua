...
function winbar()
    ...
    table.sort(names, function(a, b) return tabs[a].id < tabs[b].id end)
    ...
    for _, name in ipairs(names) do
        ...
        tab =
            tab .. " " .. hl .. " " .. name .. " (" .. tostring(tabs[name].id) ..
                ") " .. "%#Normal#"
    end
...
function P.create_tab(name)
    if not tabs[name] then
        ...
        if not current_tab then current_tab = name end
    end
...
function panel()
    ...
    local tab = tabs[current_tab] or
                    {id = 0, bufnr = vim.api.nvim_create_buf(false, true)}
...
vim.api.nvim_create_user_command("OutputPanel", function()
    if output_panel_winid and
        vim.tbl_contains(vim.api.nvim_list_wins(), output_panel_winid) then
